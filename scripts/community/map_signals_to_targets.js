...
function pageType(target) { return target.page_type || target.type || (target.slug || '').split('/').filter(Boolean)[0] || 'answer'; }
function scoreMatch(normalized, target, qWords) {
  let score = 0;
  const pt = pageType(target);
  if (target.cluster === normalized.cluster) score += 5;
  if (pt === normalized.intent_type) score += 4;
  if (pt === 'scenario' && normalized.intent_type === 'faq') score += 1;
  const tText = target._match_text || '';
  for (const word of qWords) if (tText.includes(word)) score += 1;
  return score;
}
...
function actionFor(best) {
  if (!best || best.score < 4) return 'hold_for_owner_review';
...
function run() {
  ...
  const approved = targets
    ...
    .map((t) => ({ ...t, _match_text: slugify(`${t.title || ''} ${t.page_id || ''} ${t.slug || ''} ${t.primary_query || ''}`) }));
  const queue = [];
  const approvalQueue = [];
  for (let i = 0; i < normalized.length; i++) {
    const n = normalized[i];
    const qWords = slugify(n.normalized_query).split('-').filter((w) => w.length > 3).slice(0, 12);
    const best = bestTarget(n, approved, qWords);
    const action = actionFor(best);
    const queueBase = {
      queue_id: `queue_${String(i).padStart(4, '0')}_${slugify(n.normalized_query).slice(0, 60)}`,
    ...
    if (action === 'strengthen_existing_page') {
      ...
      target.primary_query ||= n.normalized_query;
      target.supporting_queries = Array.from(new Set([...(target.supporting_queries || []), n.normalized_query])).slice(0, 12);
      target.provenance_status = 'source_backed';
      target.signal_score = Math.max(Number(target.signal_score || 0), Number(n.signal_score || 0));
      queue.push({ ...queueBase, mapped_slug: target.slug, status: 'approved_for_content', reason: 'Evergreen FAQ/scenario/comparison strengthening is auto-approved.' });
    } else {
      n.status = 'queued';
      const proposedSlug = `/${n.intent_type === 'comparison' ? 'compare' : n.intent_type}/${slugify(n.normalized_query).slice(0, 90)}/`;
      queue.push({ ...queueBase, mapped_slug: proposedSlug, status: 'pending_owner_review', reason: 'No strong existing evergreen target matched.' });
      approvalQueue.push({ approval_id: `approval_${String(i).padStart(4, '0')}_${slugify(n.normalized_query).slice(0, 50)}`, type: 'new_page_or_cluster', status: 'pending', normalized_id: n.normalized_id, proposed_slug: proposedSlug, cluster: n.cluster, question: n.normalized_query, source_signal_ids: n.source_signal_ids });
    }
  }
